...
public class BusinessSubTaskScheduler implements ISubTaskScheduler<BusinessSubTaskContext>,IWorkItem {
    ...
    public void execute(BusinessSubTaskContext subTaskContext) {
        ...
        try {
            ...
            /*Map sulutionMap = subTaskContext.getSolutionIds();
            IRuleMatch ruleMatch = new RuleMatchImpl();
            IDataMatcher dataMatcher = new DataMatcherImpl();
            this.logger.info("标准表方案网元信息匹配开始--->>>>" + new Date());
            for (String stdTableName : sulutionMap.keySet()) {
                List solutions = (List)sulutionMap.get(stdTableName);

                Map ruleMatchResult = ruleMatch.match(subTaskContext, solutions);

                matchResult.putAll(dataMatcher.dataMatch(subTaskContext, stdTableName, ruleMatchResult));
            }*/

            this.logger.info("标准表方案网元信息地市匹配结束--->>>>" + new Date());
            ...
            Map curCheckDataMap = curCheckData.createCurNetCheckDatas(subTaskContext, subTaskContext.getDbFile(), matchResult, subTaskContext.getNe());
            ...
            Map stdCheckDataMap = stdCheckData
                    .createSourceCheckData(subTaskContext, matchResult, curCheckDataMap);
    ...
    {
        return this.subTaskContext.getTaskId() + ";" + this.subTaskContext.getNe().getName();
    }
}

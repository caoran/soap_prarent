...
public class BusinessCurNetDataImpl implements IBusinessCurNetData {
    ...
    public List<Map<String, String>> getCurNetDataInfo(Ne neElement,  String logTableName)
    {
        List curNetDataList = new ArrayList();
        try {
            curNetDataList = getCurNetDatas(logTableName, neElement.getName());
    ...
    public List<Map<String, String>> getStdDataInfo(BusiDict busiDict, String[] neNames)
    {
        List curNetDataList = new ArrayList();
        try {
            String neStr="";
            for (String netmp:neNames){
                neStr+=",'"+netmp+"'";
            }
            if (StringUtils.isNotEmpty(neStr)){
                neStr=neStr.substring(1);
            }
            curNetDataList = getStdDatas(busiDict.getCurTable(), neStr);
        } catch (ClassNotFoundException e) {
            e.printStackTrace();
        } catch (InstantiationException e) {
            e.printStackTrace();
    ...
    private List<Map<String, String>> getCurNetDatas( String logTableName, String neElementName)
            throws ClassNotFoundException, InstantiationException, IllegalAccessException{

         DataCurNetService service = (DataCurNetService)DBServiceManager.getInstance()
            .getDBService("dataCurNetService");

        String querySql = " select  *  from  "+logTableName+"  where  devicename='"+neElementName+"'";

        if (this.logger.isDebugEnabled()) {
        this.logger.debug("通过指令字典ID获取指令参数信息->" + querySql);
    }

        return service.getCurNetDatas(querySql, "");
    }
    private List<Map<String, String>> getStdDatas( String logTableName, String neElementName)
            throws ClassNotFoundException, InstantiationException, IllegalAccessException{

         DataCurNetService service = (DataCurNetService)DBServiceManager.getInstance()
            .getDBService("dataCurNetService");

        String querySql = " select  *  from  "+logTableName+"  where devicename  in ( "+neElementName+" )";
        if (this.logger.isDebugEnabled()) {
        this.logger.debug("通过指令字典ID获取指令参数信息->" + querySql);
    }
